...
export async function authenticate(c: Context, next: Next): Promise<Response | void> {
  ...
  const publicPaths = ['/', '/rss.xml', '/atom.xml', '/feed.json', '/raw.json', '/api/items'];
  ...
  if (!authHeader) {
    return c.json({ 
      ...
      message: 'Please provide an API secret in the Authorization header using the format: Bearer <secret>'
    }, 401);
  }
